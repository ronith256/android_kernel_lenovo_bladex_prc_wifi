...
#define DRIVER_VERSION				"4.047.6.1202"
...
static void get_uc_command_str(u32 seq_cmd, char *cmd_str, int len)
{
	...
	case DBMDX_UC_SEQ_CMD_SET_ASRP_GAIN:
		...
		break;
	case DBMDX_UC_SEQ_CMD_ASRP_RECORD:
		snprintf(cmd_str, len, "ASRP Record");
		break;
...
static int send_asrp_record_config(struct dbmdx_private *p)
{
	...
	if (!(p->va_debug_mode & DBMDX_DEBUG_MODE_RECORD))
		return 0;

	for (i = 0; i < p->pdata->asrp_record_cfg_values; i++) {

		cur_reg = (u16)((p->pdata->asrp_record_cfg_value[i] >> 16) &
								0x0fff);
		cur_val = (u16)((p->pdata->asrp_record_cfg_value[i]) & 0xffff);
		...
		ret = dbmdx_send_cmd(p, p->pdata->asrp_record_cfg_value[i],
									NULL);
		if (ret < 0) {
			dev_err(p->dev, "%s: failed to send cmd\n", __func__);
			ret = -EIO;
					...
					u32 seq_cmd)
{
	...
	case DBMDX_UC_SEQ_CMD_LOAD_ASRP_PARAMS:
		...
		break;
	case DBMDX_UC_SEQ_CMD_ASRP_RECORD:
		ret = send_asrp_record_config(p);
		if (ret < 0) {
			dev_err(p->dev,
					"%s: failed send load UC ASRP Params\n",
					__func__);
				ret = -EIO;
				goto out;
		}
...
#ifdef VA_VE_I2S_MASTER
		if (usecase->change_clock_src) {
			ret = configure_usecase_clocks(p, usecase, true);
			if (ret < 0) {
				dev_err(p->dev,
					"%s: failed to change clk src\n",
					__func__);
...
static ssize_t dbmdx_asrp_record_cfg_values_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t size)
{
	struct dbmdx_private *p = dev_get_drvdata(dev);
	...
	while ((str_p = strsep(&args, " \t")) != NULL) {
		...
		if (strncmp(str_p, "index=", 6) == 0) {
			...
			if (ret) {
				...
				ret = -EINVAL;
				goto print_usage;
			} else if (val >=  pdata->asrp_record_cfg_values) {
				dev_err(p->dev, "%s: index out of range: %d\n",
					__func__, (int)val);
				ret = -EINVAL;
				goto print_usage;
	...
	p->pdata->asrp_record_cfg_value[index] = new_value;

	dev_info(p->dev, "%s: asrp_record_cfg_value[%u] was set to %u\n",
		__func__, index, new_value);

	p->unlock(p);

	for (i = 0; i < pdata->asrp_record_cfg_values; i++)
		dev_dbg(dev, "%s: ASRP Record cfg %8.8x: 0x%8.8x\n",
			__func__, i, pdata->asrp_record_cfg_value[i]);

	return size;
print_usage:
	dev_info(p->dev,
		"%s: Usage: index=[0-%u] value=newval\n",
		__func__, (u32)(pdata->asrp_record_cfg_values));
	return ret;
}


static ssize_t dbmdx_asrp_record_cfg_values_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct dbmdx_private *p = dev_get_drvdata(dev);
	...
	for (i = 0; i < pdata->asrp_record_cfg_values; i++)
		off += snprintf(buf + off, PAGE_SIZE - off,
				"\tASRP Record cfg %8.8x: 0x%8.8x\n",
				i, pdata->asrp_record_cfg_value[i]);
...
static DEVICE_ATTR(asrp_record_cfg_values, S_IRUGO | S_IWUSR,
		   dbmdx_asrp_record_cfg_values_show,
		   dbmdx_asrp_record_cfg_values_store);
...
static struct attribute *dbmdx_va_ve_common_attributes[] = {
	...
	&dev_attr_asrp_record_cfg_values.attr,
	NULL,
};
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_info(p->dev, "%s: set Usecase Manager complete cmd=0x%x\n", __func__, value);
	p->usr_op_unlock(p);
...
#ifdef CONFIG_OF
	if (p->pdata->asrp_record_cfg_values)
		kfree(p->pdata->asrp_record_cfg_value);
#endif
...
int dbmdx_set_bb_clk3(bool sign)
{
	int ret = 0;
	...
	if (sign) {
		pr_info("%s: bb_clk3 enable\n", __func__);
		ret = clk_prepare_enable(bb_clk3);
		if (ret) {
			pr_err("%s: error enable bb_clk3\n", __func__);
		}
	} else {
		pr_info("%s: bb_clk3 disable\n", __func__);
		clk_disable_unprepare(bb_clk3);
	}

	return ret;
}
EXPORT_SYMBOL_GPL(dbmdx_set_bb_clk3);
...
static int dbmdx_common_probe(struct dbmdx_private *p)
{
	...
	p->gpio_1v1_en = of_get_named_gpio(np, "en_1v1_gpio", 0);
	p->gpio_1v2_en = of_get_named_gpio(np, "en_1v2_gpio", 0);
	p->gpio_1v8_en = of_get_named_gpio(np, "en_1v8_gpio", 0);

	p->es7210_3v3_regulator = NULL;
	if (hw_get_platform_type() > HW_PLATFORM_DVT1) {
		dev_info(p->dev, "%s: above DVT1, get es7210 3v3 regulator\n", __func__);
		p->es7210_3v3_regulator = devm_regulator_get(p->dev, "es7210-3v3");
		if (IS_ERR(p->es7210_3v3_regulator)) {
			dev_err(p->dev, "%s: Can't get es7210 3v3 regulator\n", __func__);
			p->es7210_3v3_regulator = NULL;
			goto err;
		}
	}

	bb_clk3= devm_clk_get(p->dev, "bb_clk3");
	if (IS_ERR(bb_clk3)) {
		pr_err("%s: error get bb_clk3\n", __func__);
		ret = PTR_ERR(bb_clk3);
		goto err;
	}
#endif
	if (!gpio_is_valid(p->gpio_1v1_en)) {
		dev_err(p->dev, "%s: 1v1_en gpio invalid\n", __func__);
		ret = -EINVAL;
		goto err_1v1_en;
	}
	ret = gpio_request(p->gpio_1v1_en, "DBMDX 1v1_en");
	if (ret < 0) {
		dev_err(p->dev, "%s: error requesting 1v1_en gpio\n", __func__);
		goto err_1v1_en;
	}
	gpio_direction_output(p->gpio_1v1_en, 0);

	if (!gpio_is_valid(p->gpio_1v2_en)) {
		dev_err(p->dev, "%s: 1v2_en gpio invalid\n", __func__);
		ret = -EINVAL;
		goto err_1v2_en;
	}
	ret = gpio_request(p->gpio_1v2_en, "DBMDX 1v2_en");
	if (ret < 0) {
		dev_err(p->dev, "%s: error requesting 1v2_en gpio\n", __func__);
		goto err_1v2_en;
	}
	gpio_direction_output(p->gpio_1v2_en, 0);

	/* es7210 3v3 regulator */
	if (p->es7210_3v3_regulator) {
		regulator_set_voltage(p->es7210_3v3_regulator, 3300000, 3300000);
		if (ret < 0) {
			dev_err(p->dev, "%s: es7210 3v3 regulator set voltage failed\n",
				__func__);
			goto err_3v3_en;
		}
		ret = regulator_set_load(p->es7210_3v3_regulator, 100000);
		if (ret < 0) {
			dev_err(p->dev, "%s: es7210 3v3 regulator set load failed\n",
				__func__);
			goto err_3v3_en;
		}
	}

	if (!gpio_is_valid(p->gpio_1v8_en)) {
		dev_err(p->dev, "%s: 1v8_en gpio invalid\n", __func__);
		ret = -EINVAL;
		goto err_1v8_en;
	}
	ret = gpio_request(p->gpio_1v8_en, "DBMDX 1v8_en");
	if (ret < 0) {
		dev_err(p->dev, "%s: error requesting 1v8_en gpio\n", __func__);
		goto err_1v8_en;
	}
	gpio_direction_output(p->gpio_1v8_en, 0);

	gpio_set_value(p->gpio_1v1_en, 1);
	gpio_set_value(p->gpio_1v2_en, 1);
	if (p->es7210_3v3_regulator) {
		ret = regulator_enable(p->es7210_3v3_regulator);
		if (ret != 0) {
			dev_err(p->dev, "%s: Failed to enable es7210 3v3 regulator: %d\n",
				__func__, ret);
			goto err_3v3_en2;
		}
	}
	gpio_set_value(p->gpio_1v8_en, 1);

	udelay(1000);
	ret = pinctrl_select_state(p->pinctrl, p->pinctrl_open);
	if (ret) {
		dev_err(p->dev, "%s: error set 32k clock enable\n", __func__);
		goto err_32k_clk;
	}
	ret = clk_prepare_enable(bb_clk3);
	if (ret) {
		dev_err(p->dev, "%s: error enable bb_clk3\n", __func__);
		goto err_bb_clk3;
	}
...
err_clk_off:
	...
	clk_disable_unprepare(bb_clk3);
err_bb_clk3:
	pinctrl_select_state(p->pinctrl, p->pinctrl_close);
err_32k_clk:
err_3v3_en2:
	gpio_free(p->gpio_1v8_en);
err_1v8_en:
err_3v3_en:
	gpio_free(p->gpio_1v2_en);
err_1v2_en:
	gpio_free(p->gpio_1v1_en);
err_1v1_en:
	if (p->es7210_3v3_regulator) {
		regulator_set_load(p->es7210_3v3_regulator, 0);
		regulator_put(p->es7210_3v3_regulator);
	}
#ifdef CONFIG_OF
err:
#endif
		...
		struct dbmdx_private *p)
{
	...
	p->pinctrl = devm_pinctrl_get(p->dev);
	if (IS_ERR_OR_NULL(p->pinctrl)) {
		dev_err(p->dev, "%s: Cannot get D5P 32k clock pinctrl:%ld\n",
			__func__, PTR_ERR(p->pinctrl));
		ret = -EINVAL;
		goto err;
	}

	p->pinctrl_open = pinctrl_lookup_state(p->pinctrl, "open");
	if (IS_ERR_OR_NULL(p->pinctrl_open)) {
		dev_err(p->dev, "%s: Cannot get default pinctrl state:%ld\n",
			__func__, PTR_ERR(p->pinctrl_open));
		ret = -EINVAL;
		goto pinctrl_err;
	}

	p->pinctrl_close = pinctrl_lookup_state(p->pinctrl, "close");
	if (IS_ERR_OR_NULL(p->pinctrl_close)) {
		dev_err(p->dev, "%s: Cannot get sleep pinctrl state:%ld\n",
			__func__, PTR_ERR(p->pinctrl_close));
		ret = -EINVAL;
		goto pinctrl_err;
	}

	ret = pinctrl_select_state(p->pinctrl, p->pinctrl_close);
	if (ret) {
		dev_err(p->dev, "%s: start pinctrl set sleep failed\n", __func__);
		goto pinctrl_err;
	}
	...
	if (pdata->feature_va_ve) {
		...
		property = of_find_property(np, "asrp_record_config",
			&pdata->asrp_record_cfg_values);

		if (property) {
			pdata->asrp_record_cfg_value = kzalloc(
				pdata->asrp_record_cfg_values, GFP_KERNEL);
			if (!pdata->asrp_record_cfg_value) {
				ret = -ENOMEM;
				goto out_err;
			}
			pdata->asrp_record_cfg_values /= sizeof(u32);
			ret = of_property_read_u32_array(np,
						 "asrp_record_config",
						 pdata->asrp_record_cfg_value,
						 pdata->asrp_record_cfg_values);
			if (ret) {
				dev_err(dev,
					"%s: Couldn't read asrp_record conf.\n",
					__func__);
				ret = -EIO;
				goto out_err;
			}
			...
			dev_info(dev,
				"%s: using %u ASRP Record conf. values from dev-tree\n",
					__func__,
					pdata->asrp_record_cfg_values);

			for (i = 0; i < pdata->asrp_record_cfg_values; i++)
				dev_dbg(dev,
					"%s: ASRP Record cfg %8.8x: 0x%8.8x\n",
					__func__, i,
					pdata->asrp_record_cfg_value[i]);
		}
...
/* check if enabled features make sense */
	...
	for (i = 0; i < pdata->asrp_record_values; i++)
		dev_dbg(dev, "%s: ASRP Record cfg %8.8x: 0x%8.8x\n",
				__func__, i, pdata->asrp_record_cfg_value[i]);
...
int dbmdx_probe_status(void)
{
	return probe_status;
}
EXPORT_SYMBOL_GPL(dbmdx_probe_status);
...
static int dbmdx_platform_probe(struct platform_device *pdev)
{
	...
	dbmdx_snd_init(p);
#endif
	register_hardware_info("Soundtrigger-DSP", "DSPG-DBMD5P");
