...
//static int tfa98xx_fw_ok_count = 0;
...
static int cal_impedence[4] = { 0, 0, 0, 0 };

extern int register_hardware_info(const char *name, const char *model);
extern int dbmdx_probe_status(void);
...
static int tfa98xx_open_device(struct tfa98xx *tfa98xx)
{
...
static int tfa98xx_close_device(struct tfa98xx *tfa98xx)
{
	dev_info(tfa98xx->dev, "%s enter\n", __func__);
...
static enum Tfa98xx_Error tfa98xxCalibration(struct tfa98xx *tfa98xx, int *speakerImpedance) {
	...
	tfa_dev_set_state(tfa98xx->tfa, TFA_STATE_OPERATING, 1);
	if (tfa_dev_mtp_get(tfa98xx->tfa, TFA_MTP_EX) ) {
		...
		if (err) {
			pr_err("tfa_dsp_get_calibration_impedance failed %d ", err);
			return err;
		}
	}
	else
	...
	if (cal_profile >= 0) {
		...
		if (err) {
			pr_err("tfaContWriteProfile failed %d", err);
			return err;
		}
	}
	...
	if (err) {
		pr_err("tfa_set_filters failed %d", err);
		return err;
	}
		...
		size_t count, loff_t *ppos)
{
	...
	if (sscanf(buf, "%d", &value) == 1) {
		if(value == 1){
			tfa98xxCalibration(tfa98xx, &cal_impedence[tfa98xx->i2c->addr - 0x34]);
		...
		size_t count, loff_t *ppos)
{
	struct i2c_client *i2c = file->private_data;
	struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);
	int tmpi, kt, it, ret = 0;
	char string[25], tempc[25];
	tmpi = cal_impedence[tfa98xx->i2c->addr - 0x34];
	it = 0;
	while(tmpi > 0)
	{
		tempc[it] = '0' + (tmpi % 10);
	...
	if (cal_impedence[tfa98xx->i2c->addr - 0x34] == 0) {
		string[0] = '0';
		kt = 1;
	}
	string[kt] = '\n';
	string[kt + 1] = '\0';
	//itoa(cal_impedence, string, 10);

    //printf("integer=%d string=%s\n", cal_impedence, string);
	ret = simple_read_from_buffer((void *)user_buf, count, ppos, string, strlen(string));
	return ret;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (profile < 0) {
		pr_err("invalid profile %d (mixer_profile=%d, rate=%d)\n",
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		tfacont_get_max_vstep(tfa98xx->tfa, profile) - vstep - 1;
	}
	mutex_unlock(&tfa98xx_mutex);
...
static int tfa98xx_info_dev_select_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	mutex_lock(&tfa98xx_mutex);
...
static int tfa98xx_get_dev_select_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct tfa98xx *tfa98xx;
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		int i = tfa98xx->tfa->dev_idx;
		mutex_lock(&tfa98xx->dsp_lock);
		ucontrol->value.integer.value[i] = tfa98xx->tfa->is_open;
		mutex_unlock(&tfa98xx->dsp_lock);
		dev_info(tfa98xx->dev, "%s: value = %d\n",
			__func__, ucontrol->value.integer.value[i]);
	}
	mutex_unlock(&tfa98xx_mutex);
...
static int tfa98xx_set_dev_select_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct tfa98xx *tfa98xx;
	int dev_open_num = 0;
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		int i = tfa98xx->tfa->dev_idx;
		mutex_lock(&tfa98xx->dsp_lock);
		tfa98xx->tfa->is_open = (uint16_t)ucontrol->value.integer.value[i];
		mutex_unlock(&tfa98xx->dsp_lock);
		dev_info(tfa98xx->dev, "%s: tfa98xx is open = %d\n",
			__func__, tfa98xx->tfa->is_open);

		if (tfa98xx->tfa->is_open) {
			tfa98xx_open_device(tfa98xx);
			dev_open_num++;
		} else {
			tfa98xx_close_device(tfa98xx);
		}
	}

	mutex_lock(&tfa98xx_mutex);
	if ((dev_open_num == 0) || (dev_open_num == 4)) {
		tfa_mode = AP_MODE;
	} else {
		tfa_mode = FACTORY_MODE;
	}
	mutex_unlock(&tfa98xx_mutex);
...
static int tfa98xx_info_do_calibrate_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	mutex_lock(&tfa98xx_mutex);
...
static int tfa98xx_get_calibration_result_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct tfa98xx *tfa98xx;
	...
	list_for_each_entry_reverse(tfa98xx, &tfa98xx_device_list, list) {
		...
		pr_info("tfa98xx-0x%x: get calibration impedance : %d ",
			tfa98xx->i2c->addr, cal_impedence[i]);
	}
	return 0;
}

static int tfa98xx_set_do_calibrate_ctl(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct tfa98xx *tfa98xx;
	mutex_lock(&tfa98xx_mutex);
	pr_info("%s enter.", __func__);
#if 0
	if (!pa_calibration_work_is_inited) {
		pa_calibration_work_is_inited = true;
		INIT_DELAYED_WORK(&pa_calibration_work, tfa98xx_factory_calibration_work);
	}
#else
	list_for_each_entry_reverse(tfa98xx, &tfa98xx_device_list, list) {
		int i = tfa98xx->tfa->dev_idx;
		if (ucontrol->value.integer.value[i] == 1) {
			cal_impedence[i] = 0;
			pr_info("tfa98xx-0x%x: tfa calibration started!",
				tfa98xx->i2c->addr);
			//tfa98xxCalibration(tfa98xx, &cal_impedence[i]);
			queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->calibration_work, msecs_to_jiffies(1));
		}
	}
#endif
	mutex_unlock(&tfa98xx_mutex);
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	nr_controls = 4;/* Profile, stop, select device and do calibrate control */
	...
	for (prof = 0; prof < nprof; prof++) {
		 ...
		 * if it is not found then add it to the list and add a
		 * new mixer control (if it has vsteps) also, if it is
		 * a calibration profile, do not add it to the list
		 */
	...
	/* Create a mixer item for device select control */
	name = devm_kzalloc(tfa98xx->component->dev, MAX_CONTROL_NAME, GFP_KERNEL);
	if (!name)
		return -ENOMEM;

	scnprintf(name, MAX_CONTROL_NAME, "TFA Device Select");
	tfa98xx_controls[mix_index].name = name;
	tfa98xx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	tfa98xx_controls[mix_index].info = tfa98xx_info_dev_select_ctl;
	tfa98xx_controls[mix_index].get = tfa98xx_get_dev_select_ctl;
	tfa98xx_controls[mix_index].put = tfa98xx_set_dev_select_ctl;
	mix_index++;

	/* Create a mixer item for pa calibrate control */
	name = devm_kzalloc(tfa98xx->component->dev, MAX_CONTROL_NAME, GFP_KERNEL);
	if (!name)
		return -ENOMEM;
	scnprintf(name, MAX_CONTROL_NAME, "Do Calibrate");
	tfa98xx_controls[mix_index].name = name;
	tfa98xx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	tfa98xx_controls[mix_index].info = tfa98xx_info_do_calibrate_ctl;
	tfa98xx_controls[mix_index].get = tfa98xx_get_calibration_result_ctl;
	tfa98xx_controls[mix_index].put = tfa98xx_set_do_calibrate_ctl;
	mix_index++;
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	if ((tfa98xx->tfa->tfa_family == 2) && (0 == tfa98xx->tfa->is_probus_device)) {
		...
		ret = tfa_dev_stop(tfa98xx->tfa);
		if (ret != tfa_error_ok)
			pr_err("stop TFA device is failed!\n");
		tfa98xx->dsp_init = TFA98XX_DSP_INIT_STOPPED;
...
static void tfa98xx_factory_calibration_work(struct work_struct *work)
{
	struct tfa98xx *tfa98xx;
	tfa98xx = container_of(work, struct tfa98xx, calibration_work.work);
	tfa98xxCalibration(tfa98xx, &cal_impedence[tfa98xx->tfa->dev_idx]);
}
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (sync) {
		...
		if (tfa_mode == FACTORY_MODE) {
			do_sync = 1;
			dev_info(&tfa98xx->i2c->dev, "%s: factory mode, set do_sync=1\n", __func__);
		} else {
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (tfa_mode != AP_MODE) {
		pr_err("%s: is not ap mode(%d), return\n", __func__, tfa_mode);
		return 0;
	}
...
//add for tv mode
int tfa98xx_set_pa_state(int on)
{
	...
	if (on) {
		pr_info("%s: set to TV mode", __func__);
		mutex_lock(&tfa98xx_mutex);
		tfa_mode = TV_MODE;
		mutex_unlock(&tfa98xx_mutex);
	}
	...
	if (!on) {
		pr_info("%s: set to AP mode", __func__);
		mutex_lock(&tfa98xx_mutex);
		tfa_mode = AP_MODE;
		mutex_unlock(&tfa98xx_mutex);
	}

	pr_info("%s: tfa on=%d end\n", __func__, on);

	return 0;
}
EXPORT_SYMBOL_GPL(tfa98xx_set_pa_state);
...
static int tfa98xx_probe(struct snd_soc_component *component)
{
	...
	dev_info(component->dev, "%s: tfa98xx codec registered (%s)",
		__func__, tfa98xx->fw.name);

//TV framework to handle
#if 0
	if (tfa_mode == TV_MODE) {
		list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
			if (tfa98xx->dsp_fw_state == TFA98XX_DSP_FW_OK) {
				tfa98xx_fw_ok_count++;
				pr_info("%s: dsp firmware ok, tfa98xx_fw_ok_count = %d\n",
					__func__, tfa98xx_fw_ok_count);
			}
		}
		if (tfa98xx_fw_ok_count == 4) {
			pr_info("%s: set pa on. \n", __func__);
			tfa98xx_set_pa_state(true);
		} else {
			pr_info("%s: not all tfa firmware loaded\n", __func__);
			tfa98xx_fw_ok_count = 0;
		}
	}
#endif
	return ret;
...
static void tfa98xx_remove(struct snd_soc_component *component)
{
	...
	cancel_delayed_work_sync(&tfa98xx->init_work);
	cancel_delayed_work_sync(&tfa98xx->tapdet_work);
	cancel_delayed_work_sync(&tfa98xx->calibration_work);
	...
	struct device_node *np)
{
	if (hw_get_platform_type() <= HW_PLATFORM_DVT1) {
		tfa98xx->vdd = regulator_get(tfa98xx->dev, "vdd");
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (dbmdx_probe_status() == 0) {
		pr_info("%s: tfa9894 depend on D5P probe, defer...\n", __func__);
		return -EPROBE_DEFER;
	}
	...
	if (tfa98xx->vdd) {
		ret = regulator_set_load(tfa98xx->vdd, 200000);
		if (ret < 0)
			pr_err("%s: regulator set load failed\n", __func__);
	...
	INIT_DELAYED_WORK(&tfa98xx->calibration_work, tfa98xx_factory_calibration_work);
	...
	register_hardware_info("PA", "NXP-TFA9894");
