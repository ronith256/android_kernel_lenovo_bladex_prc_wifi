...
//#define use_msleep_interruptible
...
enum tfa_error tfa_dev_set_state(struct tfa_device *tfa, enum tfa_state state, int is_calibration)
{
	...
	int loop = 50, ready = 0;
	int count;
#ifdef use_msleep_interruptible
	int remain_ms;
#endif
	...
	case TFA_STATE_OPERATING:    /* Amp and Algo running */
		...
		pr_info("tfa->is_probus_device=%d    is_calibration=%d\n", tfa->is_probus_device, is_calibration);
	if (!tfa->is_probus_device)
		 ...
		 */
		pr_err("MTPOTC:%d ", TFA_GET_BF(tfa, MTPOTC));
		if ((TFA_GET_BF(tfa, MTPOTC) == 1) && (tfa->tfa_family == 2) && is_calibration) {
			count = MTPEX_WAIT_NTRIES * 4; /* Calibration takes a lot of time */
			pr_err("wait dsp calibration start . count:%d ", count);
			while ((TFA_GET_BF(tfa, MTPEX) != 1) && count) {
#ifdef use_msleep_interruptible
				remain_ms = msleep_interruptible(10);
				if (remain_ms != 0) {
					pr_err("sleep interrupt!! remain %d ms .", remain_ms);
					msleep(remain_ms);
				}
#else
				msleep(10);
			...
			pr_err("wait dsp calibration finished done. count:%d ", count);
		}
		if (((!tfa->is_probus_device) && (is_calibration)) || ((tfa->rev & 0xff) == 0x13)) {
