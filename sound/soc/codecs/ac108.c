...
#define AC108_REGISTER_CODEC		0		//AC108 register codec
...
struct ac108_priv {
	...
	struct device *dev;
#if AC108_REGISTER_CODEC
	struct snd_soc_codec *codec;
#endif//AC108_REGISTER_CODEC
...
static void ac108_hw_init(struct i2c_client *i2c)
{
	...
	ac108_write(0x90, 0x3c, i2c);
	ac108_write(0x91, 0x3c, i2c);
	ac108_write(0x92, 0x3c, i2c);
	ac108_write(0x93, 0x3c, i2c);
...
#if 0
static void ac108_hw_init_2channel(struct i2c_client *i2c)
{
	ac108_write(0x00, 0x12, i2c);
	msleep(100);
	ac108_write(0x06, 0x01, i2c);
	...
	ac108_write(0x09, 0x81, i2c);
	ac108_write(0x10, 0x4f, i2c);
	ac108_write(0x11, 0x00, i2c);
	ac108_write(0x12, 0x01, i2c);
	ac108_write(0x13, 0x40, i2c);
	ac108_write(0x14, 0x29, i2c);
	ac108_write(0x18, 0x01, i2c);
	ac108_write(0x20, 0x99, i2c);
	ac108_write(0x21, 0x93, i2c);
	ac108_write(0x22, 0x93, i2c);
	...
	ac108_write(0x38, 0x01, i2c);
	ac108_write(0x39, 0x03, i2c);
	ac108_write(0x3c, 0x04, i2c);
	ac108_write(0x60, 0x08, i2c);
	ac108_write(0x61, 0x1f, i2c);
	ac108_write(0x66, 0x0f, i2c);
	ac108_write(0x7f, 0x00, i2c);
	ac108_write(0x90, 0x28, i2c);
	ac108_write(0x91, 0x28, i2c);
	ac108_write(0x92, 0x28, i2c);
	ac108_write(0x93, 0x28, i2c);
	ac108_write(0xa0, 0x07, i2c);
	ac108_write(0xa7, 0x07, i2c);
	ac108_write(0xae, 0x07, i2c);
	ac108_write(0xb4, 0x03, i2c);
	ac108_write(0xb5, 0x07, i2c);
	ac108_write(0xbb, 0x0f, i2c);
}
#endif

static void ac108_hw_init_48k_32b_2ch(struct i2c_client *i2c)
{
	ac108_write(0x00, 0x12, i2c);
	msleep(10);
	ac108_write(0x06, 0x01, i2c);
	...
	ac108_write(0x22, 0x93, i2c);
	ac108_write(0x30, 0x15, i2c);
	ac108_write(0x33, 0x1f, i2c);
	ac108_write(0x34, 0x1d, i2c);
	ac108_write(0x35, 0x77, i2c);
	ac108_write(0x38, 0x01, i2c);
	ac108_write(0x39, 0x03, i2c);
	ac108_write(0x3c, 0x04, i2c);
	ac108_write(0x44, 0x0e, i2c);
	ac108_write(0x60, 0x08, i2c);
	ac108_write(0x61, 0x1f, i2c);
	ac108_write(0x66, 0x0f, i2c);
	ac108_write(0x7f, 0x00, i2c);
...
#endif
	pr_info("%s: enter ac108 suspend\n",__func__);
        if (regulator_en && !IS_ERR(ac108->vol_supply.vcc1v8) && !IS_ERR(ac108->vol_supply.ldoin3v3)) {
		ret = regulator_disable(ac108->vol_supply.ldoin3v3);
		if(ret != 0)
		        pr_err("[AC108] %s: some error happen, fail to disable regulator 3v3!\n", __func__);
		ret = regulator_disable(ac108->vol_supply.vcc1v8);
		if(ret != 0)
...
static int ac108_resume(struct snd_soc_codec *codec)
{
	...
	if (!regulator_en && !IS_ERR(ac108->vol_supply.vcc3v3)) {
        ...
        if (!regulator_en && !IS_ERR(ac108->vol_supply.vcc1v8) && !IS_ERR(ac108->vol_supply.ldoin3v3)) {
		ret = regulator_enable(ac108->vol_supply.vcc1v8);
		if(ret != 0)
			pr_err("[AC108] %s: some error happen, fail to enable regulator 1v8!\n", __func__);
		ret = regulator_enable(ac108->vol_supply.ldoin3v3);
		if(ret != 0)
...
static int ac108_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *i2c_id)
{
	...
	if (hw_get_platform_type() > HW_PLATFORM_DVT1) {
		pr_info("%s: not DVT1 or below DVT1 version, donot probe\n", __func__);
		return -ENODEV;
	}
	...
	if (!regulator_en) {
		if (hw_get_platform_type() == HW_PLATFORM_EVB) {
			ac108->vol_supply.ldoin3v3 = regulator_get(ac108->dev, "ldoin3v3-evb");
			if (IS_ERR(ac108->vol_supply.ldoin3v3) || !ac108->vol_supply.ldoin3v3) {
				pr_err("get ac108 audio-3v3 of evb failed, return!\n");
				return -EFAULT;
			}
			pr_info("%s: get regulator of ac108 evb success\n", __func__);
		}else {
			ac108->vol_supply.ldoin3v3 = regulator_get(ac108->dev, "ldoin3v3-evt");
			if (IS_ERR(ac108->vol_supply.ldoin3v3) || !ac108->vol_supply.ldoin3v3) {
				pr_err("get ac108 audio-3v3 of evt failed, return!\n");
				return -EFAULT;
			}
			pr_info("%s: get regulator of ac108 evt success\n", __func__);
		}
		regulator_set_voltage(ac108->vol_supply.ldoin3v3, 3300000, 3300000);
		ret = regulator_set_load(ac108->vol_supply.ldoin3v3, 100000);
		if (ret < 0)
			pr_err("%s: regulator ac108 3v3 set load failed\n", __func__);
		ret = regulator_enable(ac108->vol_supply.ldoin3v3);
		if(ret != 0)
			pr_err("[AC108] %s: some error happen, fail to enable regulator!\n", __func__);

#if 0
		ac108->vol_supply.vcc1v8 = regulator_get(ac108->dev, "vcc1v8");
	        if (IS_ERR(ac108->vol_supply.vcc1v8) || !ac108->vol_supply.vcc1v8) {
		        pr_err("get ac108 vcc_1v8 failed, return!\n");
		        return -EFAULT;
	        }
	        regulator_set_voltage(ac108->vol_supply.vcc1v8, 1800000, 1800000);
	        ret = regulator_enable(ac108->vol_supply.vcc1v8);
	        if(ret != 0)
		        pr_err("[AC108] %s: some error happen, fail to enable regulator vcc_1v8!\n", __func__);

	        pr_debug("%s:enable regulator vcc_1v8 successful!", __func__);
#endif
...
//	ac108_hw_init_2channel(i2c_clt[0]);
	ac108_hw_init_48k_32b_2ch(i2c_clt[0]);
	pr_info("%s: ac108 init complete\n", __func__);
...
static int ac108_suspend(struct device *dev)
{
        int ret;
	struct ac108_priv *ac108 = dev_get_drvdata(dev);
#if AC108_MATCH_DTS_EN
	...
	if (regulator_en && !IS_ERR(ac108->vol_supply.ldoin3v3)) {
		regulator_disable(ac108->vol_supply.ldoin3v3);
		regulator_en = 0;
	}
#endif
	pr_info("%s: enter ac108 suspend\n",__func__);
...
static int ac108_resume(struct device *dev)
{
	struct ac108_priv *ac108 = dev_get_drvdata(dev);
	int ret;
...
#else
#define ac108_i2c_suspend NULL
#define ac108_i2c_resume NULL
#endif /* CONFIG_PM_SLEEP */

#ifdef CONFIG_PM
static int ac108_i2c_runtime_suspend(struct device *dev)
{
	dev_dbg(dev, "%s\n", __func__);
	ac108_suspend(dev);
...
static int ac108_i2c_runtime_resume(struct device *dev)
{
	dev_dbg(dev, "%s\n", __func__);
	ac108_resume(dev);
...
#else
#define ac108_i2c_runtime_suspend NULL
#define ac108_i2c_runtime_resume NULL
#endif /* CONFIG_PM */

static const struct dev_pm_ops ac108_i2c_pm = {
	SET_SYSTEM_SLEEP_PM_OPS(ac108_i2c_suspend, ac108_i2c_resume)
	SET_RUNTIME_PM_OPS(ac108_i2c_runtime_suspend,
				ac108_i2c_runtime_resume, NULL)
};
