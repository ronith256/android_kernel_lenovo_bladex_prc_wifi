...
#include "../../../../techpack/display/msm/dsi/dsi86-edp-mux.h"
...
struct tv_report tv_touchpoint;
struct drm_panel *goodix_panel;
static struct goodix_ts_core *g_ts_core_data = NULL;
...
static int goodix_debugfs_init(void)
{
	...
	printk("goodix_debugfs_init run normal\n");
exit:
	return 0;
}
...
static void goodix_ts_tv_report_finger(struct input_dev *dev,
		struct goodix_touch_data *touch_data)
{
	unsigned int touch_num = touch_data->touch_num;
	static u32 pre_fin_tv = 0;
	static u32 irq_c = 0;
	struct timespec64 duration;
	unsigned long delta;
	int flag = 0;
	static int long_down_press = 0;
	ktime_get_ts64(&tv_touchpoint.end);
	/*first touch down and last touch up condition*/
	if (touch_num && !pre_fin_tv){
		tv_touchpoint.start = tv_touchpoint.end;
		ts_err("down %d", touch_num);
	}
	else if (!touch_num && pre_fin_tv) {
		flag = 1;
		long_down_press = 0;
		ts_err("up %d", touch_num);
	}
	pre_fin_tv = touch_num;
	duration = timespec64_sub(tv_touchpoint.end, tv_touchpoint.start);
	delta = (duration.tv_sec*1000) + duration.tv_nsec/(1000*1000);
	if (delta > 500) {  // long press
		if (irq_c%FILTER_POINT == 0) {
			if (irq_c == FILTER_POINT)
				irq_c = 0;
			if (((touch_data->coords[0].x > TV_COODS_DOWN_X_MIN) &&
				(touch_data->coords[0].x < TV_COODS_DOWN_X_MAX) &&
					(touch_data->coords[0].y > TV_COODS_Y)) ||
			    ((touch_data->coords[0].x > TV_COODS_UP_X_MIN) &&
				(touch_data->coords[0].x < TV_COODS_UP_X_MAX) &&
					(touch_data->coords[0].y > TV_COODS_Y))) {
				//ts_err("long press  X:%d, Y:%d ", touch_data->coords[0].x, touch_data->coords[0].y);
				notify_scaler_touch_point(touch_data->coords[0].x, touch_data->coords[0].y, true);
			} else if (long_down_press == 1) {
				ts_err("long press one touch X:%d, Y:%d ", touch_data->coords[0].x, touch_data->coords[0].y);
				notify_scaler_touch_point(touch_data->coords[0].x, touch_data->coords[0].y, true);
			}
			long_down_press++;
		}
		irq_c++;
	}else if ((delta < 500) && (!pre_fin_tv) && (touch_num == 0)) { // short press
		if (flag == 1) {// filter report after "up"
			ts_err("short press < 500 X:%d, Y:%d ", touch_data->coords[0].x, touch_data->coords[0].y);
			notify_scaler_touch_point(touch_data->coords[0].x, touch_data->coords[0].y, false);
			flag = 0;
		}
	}
	//ts_err("delta %d", delta);
}
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	mutex_lock(&tv_touchpoint.tv_mutex);
	//pm_stay_awake(ts_dev->dev);
	irq_tv_status = get_connection_status();
	//ts_err("irq_tv_status  %d", irq_tv_status);
	if (irq_tv_status == TV_CONNECTED) {
		pm_wakeup_event(ts_dev->dev, 5000);
		if (!tv_touchpoint.complete) {
			r = ts_dev->hw_ops->event_handler(ts_dev, ts_event);
			if (likely(r >= 0)) {
				if ((ts_event->event_type & EVENT_TOUCH) == EVENT_TOUCH) {
					/* report touch */
					goodix_ts_tv_report_finger(core_data->input_dev,
							&ts_event->touch_data);
				}
			}
		} else {
			while (1) {
				...
				if (likely(r >= 0)) {
					if ((ts_event->event_type & EVENT_TOUCH) == EVENT_TOUCH) {
						tv_touchpoint.first_report++;
						if (tv_touchpoint.first_report==1) {
							max_read_count = 0;
							ts_err("irq press  X:%d, Y:%d ", ts_event->touch_data.coords[0].x, ts_event->touch_data.coords[0].y);
							notify_scaler_touch_point(ts_event->touch_data.coords[0].x, ts_event->touch_data.coords[0].y,false);
						}
						break;
					}
				} else {
						max_read_count++;
						if (max_read_count == 200){
							max_read_count = 0;
							ts_err("> max_read_count , quit", max_read_count);
							break;
						}
						mdelay(1);
						continue;
	...
	//pm_relax(ts_dev->dev);
	mutex_unlock(&tv_touchpoint.tv_mutex);
	/* clean irq flag */
	irq_flag[0] = 0;
	irq_flag[1] = 0;
...
*/
	//printk("ts_bdata->avdden_gpio = %d\n", ts_bdata->avdden_gpio);
/*	if(r< 0){
...
static int goodix_ts_suspend(struct goodix_ts_core *core_data)
{
	 ...
	 */
	tv_touchpoint.hdmi_status = get_connection_status();
	if (tv_touchpoint.hdmi_status) {
		if (tv_touchpoint.suspend_status == 1) {
			ts_err("goodix Suspend start suspend_status %d",tv_touchpoint.suspend_status);
			goto tv_connected;
		}
		tv_touchpoint.suspend_by_hdmi = 1;
		tv_touchpoint.suspend_status = 1;
		tv_touchpoint.resume_status = 0;
		tv_touchpoint.complete = 0;
		ts_err("PLUG IN BRIGHT suspend_by_hdmi %d hdmi_status %d resume_status %d suspend_status %d complete %d",tv_touchpoint.suspend_by_hdmi,
				tv_touchpoint.hdmi_status,tv_touchpoint.resume_status,tv_touchpoint.suspend_status,tv_touchpoint.complete);
		goto tv_connected;
	}
	/* inform external module */
	mutex_lock(&goodix_modules.mutex);
	if (!list_empty(&goodix_modules.head)) {
		list_for_each_entry_safe(ext_module, next,
			...
			if (!ext_module->funcs->before_suspend)
				continue;
			if((gsx_gesture->gesture_enable == 0) && (strcmp(ext_module->name,"Goodix_gsx_gesture") == 0))
				continue;
...
static int goodix_ts_resume(struct goodix_ts_core *core_data)
{
	...
	struct goodix_ts_device *ts_dev =
				core_data->ts_dev;
	int r, resume_complete = -1;
	ts_err("goodix Resume start ");
	if (tv_touchpoint.suspend_by_hdmi) {
		if(tv_touchpoint.resume_status) {
			ts_err("goodix Resume start suspend_status %d",tv_touchpoint.resume_status);
			goto out;
		}
		tv_touchpoint.suspend_by_hdmi = 0;
		tv_touchpoint.hdmi_status = 0;
		tv_touchpoint.resume_status = 1;
		tv_touchpoint.suspend_status = 0;
		tv_touchpoint.complete = 1;
		ts_err("PLUG OUT  suspend_by_hdmi %d hdmi_status %d resume_status %d suspend_status %d complete %d",tv_touchpoint.suspend_by_hdmi,
				tv_touchpoint.hdmi_status,tv_touchpoint.resume_status,tv_touchpoint.suspend_status,tv_touchpoint.complete);
		goto out;
	} else if (tv_touchpoint.brightness_plug_in == 0) {
		tv_touchpoint.brightness_plug_in = -1;
		tv_touchpoint.suspend_by_hdmi = 1;
		tv_touchpoint.suspend_status = 1;
		tv_touchpoint.resume_status = 0;
		tv_touchpoint.hdmi_status = 1;
		tv_touchpoint.complete = 1;
		ts_err("PLUG IN suspend_by_hdmi %d hdmi_status %d resume_status %d suspend_status %d complete %d",tv_touchpoint.suspend_by_hdmi,
				tv_touchpoint.hdmi_status,tv_touchpoint.resume_status,tv_touchpoint.suspend_status,tv_touchpoint.complete);
	}
	...
	if (ts_dev && ts_dev->hw_ops->resume)
		resume_complete = ts_dev->hw_ops->resume(ts_dev);
	...
	tv_touchpoint.complete = resume_complete;
	ts_info("goodix Resume end complete = %d ",tv_touchpoint.complete);
	/*
...
int goodix_drm_notifier_callback(struct notifier_block *self,unsigned long event, void *data)
{
	 ...
	 if (event == DRM_PANEL_EARLY_EVENT_BLANK) {
		if (*blank == DRM_PANEL_BLANK_POWERDOWN) {
			cancel_delayed_work_sync(&core_data->resume_work);
			goodix_ts_suspend(core_data);
		}
...
static int goodix_ts_pm_suspend(struct device *dev)
{
	...
	struct goodix_ts_core *core_data =
		dev_get_drvdata(dev);
	ts_err("goodix_ts_pm_suspend suspend_by_hdmi %d",tv_touchpoint.suspend_by_hdmi);
	if (tv_touchpoint.suspend_by_hdmi) {
		if (get_connection_status() && (tv_touchpoint.pm_suspend_by_hdmi == 0)) {
			tv_touchpoint.complete = 1;
			tv_touchpoint.first_report = 0;
			r = enable_irq_wake(core_data->irq);
			if (r)
				ts_err("enable irq wake fail");
			tv_touchpoint.pm_suspend_by_hdmi = 1;
			ts_err("goodix_ts_pm_suspend pm_suspend_by_hdmi = %d",tv_touchpoint.pm_suspend_by_hdmi);
		}/* HDMI disconnect after LCD off */
	}
	return r;
}
/**
...
static int goodix_ts_pm_resume(struct device *dev)
{
	...
	struct goodix_ts_core *core_data =
		dev_get_drvdata(dev);
	ts_err("goodix_ts_pm_resume");
	if(tv_touchpoint.pm_suspend_by_hdmi) {
		tv_touchpoint.complete = 0;
		tv_touchpoint.pm_suspend_by_hdmi = 0;
		desc = irq_to_desc(core_data->irq);
		if (desc->wake_depth == 0) {
			ts_err("goodix Unbalanced IRQ %d wake disable\n", core_data->irq);
		} else {
			ts_info("goodix_ts_pm_resume -- disable_irq_wake");
			r = disable_irq_wake(core_data->irq);
			if (r)
...
/*#ifdef CONFIG_FB
	...
	if(goodix_panel == NULL)
		pr_err("!!!!goodix drm panel is  NULL\n");
        else if(drm_panel_notifier_register(goodix_panel, &core_data->fb_notifier)) 
...
void tp_resume_call_by_panel(void)
{

	struct goodix_ts_core *core_data = g_ts_core_data;
	if(core_data && core_data->ts_probe_complete)
		queue_delayed_work(core_data->resume_workqueue, &core_data->resume_work, 0);
}
EXPORT_SYMBOL(tp_resume_call_by_panel);

void tp_suspend_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct goodix_ts_core *core_data = container_of(dwork,
			struct goodix_ts_core, suspend_work);
	goodix_ts_suspend(core_data);
}

void tp_suspend_call_by_panel(void)
{

	struct goodix_ts_core *core_data = g_ts_core_data;
	if(core_data && core_data->ts_probe_complete)
		queue_delayed_work(core_data->suspend_workqueue, &core_data->suspend_work, 0);
}
EXPORT_SYMBOL(tp_suspend_call_by_panel);
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	core_data->ts_probe_complete = 0;
	core_data->pdev = pdev;
	core_data->ts_dev = ts_device;
	platform_set_drvdata(pdev, core_data);
	device_init_wakeup(core_data->ts_dev->dev, true);
	...
	core_data->suspend_workqueue = 	create_singlethread_workqueue("suspend_work");
	INIT_DELAYED_WORK(&core_data->suspend_work, tp_suspend_work);
	mutex_init(&tv_touchpoint.tv_mutex);
	...
	goodix_modules.core_data = core_data;
	tv_touchpoint.tv_core_data = core_data;
...
void goodix_reset_down(struct goodix_ts_core *core)
{
	gpio_direction_output(core->ts_dev->board_data.reset_gpio, 0);
	msleep(10);
}
