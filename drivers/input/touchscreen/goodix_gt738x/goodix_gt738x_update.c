...
/*****************************************************
   register hardware info no matter
   what firmware is updated success or failed or abort
*****************************************************/
int tp_register_hardware_version(struct goodix_ts_device *dev) {
	struct goodix_ts_version fw_ver;
	int ret = -EINVAL;
	char tempchar[1];
	int i =0;
	u8 cfg_ver_info = 0;
	memset(tp_hardinfo,0,64);
	memset(tempchar,0,1);

        dev->hw_ops->read(dev, 0x8050, &(cfg_ver_info), 1);
        ts_info("cfg_ver_info = %d\n",cfg_ver_info);

	ret = dev->hw_ops->read_version(dev, &fw_ver);
	if (ret) {
		ts_info("failed get active pid");
		#ifdef CONFIG_HW_BOOT_INFO
		register_hardware_info("TP","GT7387P INVALID");
		#endif
		return -EINVAL;
	}
	strcat(tp_hardinfo,"GT7387P-");
	for(i =0;i<4;i++)
	{
		sprintf(tempchar,"%02x",fw_ver.vid[i]);
	//	ts_info("tempchar %s",&tempchar);
		strcat(tp_hardinfo,tempchar);
	//	ts_info("hardwareinfo %s",tp_hardinfo);
	}
        sprintf(tempchar,", cfg_ver %d",cfg_ver_info);
        strcat(tp_hardinfo,tempchar);

	#ifdef CONFIG_HW_BOOT_INFO
	register_hardware_info("TP",tp_hardinfo);
	#endif
	return 0;
}
...
err_parse_fw:
	...
	tp_register_hardware_version(fwu_ctrl->ts_dev);
