...
#define TS_REG_I2C_RESUME               0x8000
...
static int goodix_hw_resume(struct goodix_ts_device *dev)
{
	...
	u8 data;
	//resume (i2c)
	r = dev->hw_ops->read(dev, TS_REG_I2C_RESUME, &data, 1);
	if(r < 0) {
	       ts_err("hw register 8000 :Error,static{0x8000}:%d\n",data);
	       return -EIO;
	}
	msleep(150);
	r = goodix_init_esd(dev);
	if(r < 0) {
	       ts_err("goodix_init_esd failed, r = %d\n", r);
	       return -EIO;
	}
	return r;
}
	...
	const struct i2c_device_id *dev_id)
{
	...
	ts_info("goodix_i2c_probe IN  -ts");
...
{
	ts_info("Goodix driver late  init");
