...
struct gesture_module *gsx_gesture = NULL; /*allocated in gesture init module*/
		...
		const char *buf, size_t count)
{
	...
	if (tmp == 1) {
		gsx_gesture->gesture_enable = 1;
	} else if (tmp == 0) {
		gsx_gesture->gesture_enable = 0;
	} else {
		ts_err("Parameter error!");
		return -EINVAL;
	}
